...
async function main() {
  ...
  for (let i = 0; i < slideCount; i++) {
    ...
    if (i === 4) {
      const readback = await driveRange(page, i, '1');
      interactionEvidence.push(`slide ${i + 1}: context=${readback}`);
      await page.screenshot({path: path.join(outDir, `slide-${String(i + 1).padStart(2, '0')}-interaction.png`), fullPage: false});
    }
    if (i === 6) {
      const readback = await driveRange(page, i, '2');
      interactionEvidence.push(`slide ${i + 1}: head=${readback}`);
      await page.screenshot({path: path.join(outDir, `slide-${String(i + 1).padStart(2, '0')}-interaction.png`), fullPage: false});
    }
