...
async function main() {
  ...
  await page.goto('http://127.0.0.1:9200/', {waitUntil: 'networkidle'});
  await page.screenshot({path: path.join(evidenceDir, 'editor-before-render.png')});
  const bodyText = await page.locator('body').innerText();
  if (/MathJax|Extra close brace|Missing argument|Error/i.test(bodyText)) {
  ...
  await browser.close();
  const bad = messages.filter(m => ['error', 'pageerror'].includes(m.type) || /MathJax|Extra close brace|Missing argument|Cannot read|TypeError|ReferenceError/i.test(m.text));
  if (bad.length) {
