...
export const sinTaylor = (x: number, degree: number) => {
  let s = 0;
  for (let n = 1; n <= degree; n += 2) {
    const j = (n - 1) / 2;
    s += (j % 2 ? -1 : 1) * Math.pow(x, n) / factorial(n);
  }
  return s;
};
export const localF = (x: number) => Math.exp(0.55 * x) - 1.05;
export const localFp = (x: number) => 0.55 * Math.exp(0.55 * x);
export const localFpp = (x: number) => 0.3025 * Math.exp(0.55 * x);
export const expTaylorLocal = (x: number, a: number, degree: number) => {
  let s = 0;
  for (let k = 0; k <= degree; k++) {
    s += Math.exp(0.55 * a) * Math.pow(0.55, k) * Math.pow(x - a, k) / factorial(k);
  }
  return s - 1.05;
};
